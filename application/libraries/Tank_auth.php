...
class Tank_auth
{
	...
	function login($login, $password, $remember, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->getUserPassword())) {		// password ok

					if ($user->getUserBanned() == 1) {									// fail - banned
						$this->error = array('banned' => $user->getUserBanReason());

					} else {
						$this->ci->session->set_userdata(array(
								'user_id'	=> $user->getUserId(),
								'username'	=> $user->getUserLogin(),
								'status'	=> ($user->getUserActivated() == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
						));

						if ($user->getUserActivated() == 0) {							// fail - not activated
							$this->error = array('not_activated' => '');

						} else {												// success
							if ($remember) {
								$this->create_autologin($user->getUserId());
							}
	...
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_login_available($username)) {
			...
			if (!is_null($user = $this->ci->users->create_user($data, !$email_activation))) {
	 ...
	 * @return	bool
	 */
	function is_login_available($username)
	{
		return ((strlen($username) > 0) AND $this->ci->users->is_login_available($username));
	}
	...
	function change_email($email)
	{
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, FALSE))) {
			...
			if (strtolower($user->getUserEmail()) == strtolower($email)) {		// leave activation key as is
				$data['new_email_key'] = $user->getUserNewEmailKey();
				return $data;
	...
	function forgot_password($login)
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_login($login))) {
				...
				$this->ci->users->set_password_key($user->getUserId(), $data['new_pass_key']);
				return $data;
		...
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
				...
				if ($this->ci->users->reset_password(
					...
					$this->ci->user_autologin->clear($user->getUserId());
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
			...
			$hasher = new PasswordHash(
					$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
					$this->ci->config->item('phpass_hash_portable', 'tank_auth'));
			if ($hasher->CheckPassword($old_pass, $user->getUserPassword())) {			// success
	...
	function set_new_email($new_email, $password)
	{
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
			...
			$hasher = new PasswordHash(
					$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
					$this->ci->config->item('phpass_hash_portable', 'tank_auth'));
			if ($hasher->CheckPassword($password, $user->getUserPassword())) {			// success
				...
				if ($user->getUserEmail() == $new_email) {
					$this->error = array('email' => 'auth_current_email');

				} elseif ($user->getUserNewEmail() == $new_email) {		// leave email key as is
					$data['new_email_key'] = $user->getUserNewEmailKey();
					return $data;
